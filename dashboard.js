...
function updateStats(data) {
    ...
    document.getElementById("fake-count").textContent = data.filter(r => r.verdict === "fake").length;
}
...
function renderTable(data) {
    ...
    tbody.innerHTML = data.map(r => `
        <tr>
            <td>${r.id}</td>
            <td>${r.title}</td>
            <td><span class="badge ${r.verdict}">${r.verdict === 'real' ? '🟢 Real' : '🔴 Unverified'}</span></td>
            <td>${r.realScore}%</td>
            <td>${r.fakeScore}%</td>
            <td>${r.date}</td>
